...
class GINPhi(nn.Module):
    ...
    def __init__(
        ...
        create_mlp: Callable[[int, int], MLP], args, bn: bool
    ) -> None:
        super().__init__()
        self.gin = GIN(n_layers, in_dims, hidden_dims, out_dims, create_mlp, args, bn)
        self.mlp = create_mlp(out_dims, out_dims, args)
...
#     def __init__(
#             self, n_layers: int, in_dims: int, hidden_dims: int, out_dims: int, 
#             create_mlp: Callable[[int, int], MLP], args
#     ) -> None:
#         super().__init__()
#         self.gine = GINE(n_layers, in_dims, hidden_dims, out_dims, create_mlp, args)
...
#         W = torch.cat(W_pad_list, dim=0)   # [N_sum, N_max, M]
#         PE = self.gine(W, edge_index)       # [N_sum, N_max, D_pe]
#         return PE.sum(dim=1)               # [N_sum, D_pe]

#     @property
#     def out_dims(self) -> int:
#         return self.gine.out_dims


def GetPhi(args, create_mlp: Callable[[int, int], MLP]):
    # return GINEPhi(args.n_phi_layers, args.n_psis, args.phi_hidden_dims, args.pe_dims, create_mlp, args)
    return GINPhi(args.n_phi_layers, args.n_psis, args.phi_hidden_dims, args.pe_dims,
                  create_mlp, args, args.batch_norm)


def GetPsi(args):
    return MLP(in_dims = 1, out_dims = 1, args = args, use_bn = args.mlp_use_bn)
