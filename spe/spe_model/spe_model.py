...
from .spe_gine import GINE
from .spe_mlp import MLP
from .stable_expressive_pe import StableExpressivePE, GetPhi, GetPsi
...
def construct_model(args, list_create_mlp, **kwargs): # a list of mlp creators
    ...
    base_model = GINEBaseModel(
        args.n_base_layers, args.node_emb_dims, args.base_hidden_dims, create_mlp, args,
        residual=kwargs.get("residual"), feature_type=kwargs.get('feature_type'), pooling=args.pooling,
        target_dim=target_dim
    )
    
    # pe_method
    Phi = GetPhi(args, create_mlp_ln) # for phi function, use layer norm
    Psi_list = [GetPsi(args) for _ in range(args.n_psis)]
...
class Model(nn.Module):
    ...
    def __init__(
        self, node_emb_dims: int, positional_encoding: nn.Module, base_model: nn.Module,
        ...
        self.node_features = AtomEncoder(node_emb_dims)
...
class GINEBaseModel(nn.Module):
    ...
    def __init__(
        self, n_layers: int, in_dims: int, hidden_dims: int, create_mlp: Callable[[int, int], MLP],
            args, residual: bool = False, bn: bool = False, feature_type: str = "discrete", pooling: str = "mean",
            target_dim: int = 1) -> None:
        super().__init__()
        self.gine = GINE(n_layers, in_dims, hidden_dims, hidden_dims, 
                         create_mlp, args, 
                         residual=residual, bn=bn, feature_type=feature_type)
        self.mlp = create_mlp(hidden_dims, target_dim, args)
