...
class GINE(nn.Module):
    ...
    def __init__(
        ...
        bn: bool = False, residual: bool = False, feature_type: str = "discrete"
    ) -> None:
        super().__init__()
        ...
        for _ in range(n_layers - 1):
            layer = GINELayer(in_dims, hidden_dims, create_mlp, args, feature_type)
        ...
        layer = GINELayer(hidden_dims, out_dims, create_mlp, args, feature_type)
        self.layers.append(layer)
...
class GINELayer(MessagePassing):
    ...
    def __init__(self, in_dims: int, out_dims: int, create_mlp, args, feature_type = "discrete") -> None:
        ...
        self.edge_features = BondEncoder(in_dims)
        self.pe_embedding = create_mlp(args.pe_dims, in_dims, args) # for pe-full
        # self.pe_embedding = create_mlp(37, in_dims) # for pe-full
        self.eps = torch.nn.Parameter(data=torch.randn(1), requires_grad=True)
        self.mlp = create_mlp(in_dims, out_dims, args)
