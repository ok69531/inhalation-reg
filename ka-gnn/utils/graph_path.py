...
def atom_to_graph(smiles, encoder_atom, encoder_bond):
    ...
    for i in range(len(coor)):
        ...
        for j in range(i + 1, len(coor)):
            ...
            if s_d_dis <= 5:
                if (
                    not check_common_elements(src_list, dst_list, i, j)
                    and not check_common_elements(src_list, dst_list, j, i)
                ):
                    src_list.append(i)
                    src_list.append(j)
                    dst_list.append(j)
    ...
    if len(edge_features) > 0:
        ...
        edge_feats = torch.empty((0, 0), dtype=torch.float32)
        edge_id_feats = torch.empty((0, 1), dtype=torch.float32)
